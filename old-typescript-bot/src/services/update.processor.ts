...
import { UpdateData, ProcessingResult } from '../types/index';

// Create service instances
const markdownerService = new MarkdownerService();
const airtableService = new AirtableService();

export async function processUpdate(url: string, userId: number): Promise<ProcessingResult<UpdateData>> {
  const startTime = Date.now();
  
  try {
    logger.info('Starting update processing', { url, userId });
    ...
    logger.info('Saving update to Airtable', { 
      url, 
    ...
    try {
      ...
      logger.info('Update processing completed successfully', { 
        url, 
        userId,
        updateId: recordId,
      ...
      logger.error('Failed to save update to Airtable', { 
        url, 
    ...
    const processingTime = Date.now() - startTime;
    logger.error('Unexpected error during update processing', { 
      url,
      userId,
      processingTimeMs: processingTime,
